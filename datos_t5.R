...
#Subir y estandarizar las covariables
#AGB a 30m de radio del BPI
CObuff30_aboveGbio <- read.csv("D:/Dropbox/tesis/Colbuff30_aboveGbio.csv")#win
#CObuff30_aboveGbio <- read.csv("Colbuff30_aboveGbio.csv") #Ubuntu
head(CObuff30_aboveGbio)
CObuff30_aboveGbio <- CObuff30_aboveGbio[,c(10,2)]
names(CObuff30_aboveGbio) <- c("Location","Biomass_30m")
#AGB a 100m de radio del BPI
#CObuff100_aboveGbio <- read.csv("D:/Dropbox/tesis/Colbuff100_aboveGbio.csv") #win
CObuff100_aboveGbio <- read.csv("Colbuff100_aboveGbio.csv")
head(CObuff100_aboveGbio)
CObuff100_aboveGbio <- CObuff100_aboveGbio[,c(10,2)]
names(CObuff100_aboveGbio) <- c("Location","Biomass_100m")
#Unir los dos en un solo objeto y convertirlo en data.frame
CO_Hab_BPI <- CObuff30_aboveGbio %>% left_join(CObuff100_aboveGbio)
CO_Hab_BPI2 <- as.data.frame(CO_Hab_BPI)
CO_Hab_BPI2<-arrange(CO_Hab_BPI2,Location)
#standardize covariates
CO_HAB_BPI <- CO_Hab_BPI2[c(-71),]
CO_HAB_BPI <- CO_HAB_BPI[,c(-1)] 
#hist(CO_HAB_BPI$Biomass_30m)
#hist(CO_HAB_BPI$Biomass_100m)
#Create a new data frame for prediction
newDatHab <- data.frame(Biomass_100m = mean(CO_HAB_BPI$Biomass_100m), 
                        Biomass_30m = seq(min(CO_HAB_BPI$Biomass_30m),max(CO_HAB_BPI$Biomass_30m), length=100))
#para predecir
Biomass_30m <- seq(min(CO_Hab_BPI2$Biomass_30m),max(CO_Hab_BPI2$Biomass_30m), length=100)
